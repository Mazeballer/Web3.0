...
export default function LendPage() {
  ...
  async function handleDeposit() {
    ...
    try {
      ...
      try {
        ...
        qc.invalidateQueries({ queryKey: ['trustPoints', userEmail] });
      } catch (error) {
  ...
  const handleWithdraw = async () => {
    ...
    const depositedSec = Math.floor(
      new Date(selectedPool.deposited_at).getTime() / 1000
    );
    const elapsedYears = (nowSec - depositedSec) / (365 * 24 * 60 * 60);
    const apyDecimal = selectedPool.apy / 100;
    const interest = max * apyDecimal * Math.max(elapsedYears, 0);
    const requiredPayout = max + interest;

    if (selectedPool.availableLiquidity + 1e-12 < requiredPayout) {
      return toast({
        title: 'Insufficient pool liquidity',
        description: `Available: ${selectedPool.availableLiquidity.toFixed(
          4
        )} ${selectedPool.token}. Required: ${requiredPayout.toFixed(4)} ${
          selectedPool.token
        }. Try again later.`,
        variant: 'destructive',
      });
